...
public sealed class EscapeCancellableInput(IAnsiConsoleInput originalInput) : IAnsiConsoleInput
{
    ...
    {
        if (StraumrKeybinds.Get("Cli.Cancel")?.Matches(key) == true)
        {
        ...
        if (Matches("Cli.Next") || Matches("Cli.NextUpper"))
        {
            return Navigation(ConsoleKey.DownArrow);
        }

        if (Matches("Cli.Previous") || Matches("Cli.PreviousUpper"))
        {
            return Navigation(ConsoleKey.UpArrow);
        }

        if (Matches("Cli.First"))
        {
            return Navigation(ConsoleKey.Home);
        }

        if (Matches("Cli.Last"))
        {
            return Navigation(ConsoleKey.End);
        }

        if (Matches("Cli.Search"))
        {
            return Activate();
        }

        return char.IsControl(key.KeyChar) ? key : null;

        bool Matches(string id) => StraumrKeybinds.Get(id)?.Matches(key) == true;
        static ConsoleKeyInfo Navigation(ConsoleKey target) => new('\0', target, false, false, false);
    }
