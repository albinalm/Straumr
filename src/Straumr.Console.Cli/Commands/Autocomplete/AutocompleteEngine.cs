...
internal sealed class AutocompleteEngine(IServiceProvider services)
{
    private static readonly HashSet<string> WorkspaceIdentifierVerbs =
        new([
                CompletionCatalog.Get, CompletionCatalog.Delete, CompletionCatalog.Edit,
    ...
    private static AutocompleteParsedQueryModel Parse(string query)
    {
        List<string> tokens = [];
        ...
        return new AutocompleteParsedQueryModel(tokens, !tokenStarted);
    }
}
