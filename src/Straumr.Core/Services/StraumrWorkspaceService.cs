    ...
    : IStraumrWorkspaceService
{
    ...
    {
        List<StraumrWorkspace> workspaces = new();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        StraumrWorkspace sourceWorkspace = await ReadWorkspaceAsync(
            sourceEntry.Path, false, cancellationToken);
    ...
    private string GetWorkspaceRoot(string? outputDir = null)
    {
        ...
        return settingsService.DefaultWorkspacePath
               ?? throw new StraumrException(
                   "No default workspace path configured. Pass an output directory, or set workspaces under [paths] in settings.toml",
