    ...
    IHttpClientFactory httpClientFactory) : IStraumrAuthService
{
        ...
        CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        StraumrWorkspace workspaceModel = await LoadWorkspaceAsync(workspace, cancellationToken);
        AuthLookupModel lookup = await RequireAuthAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var httpMessage = authRequest.ToHttpRequestMessage(null);
    ...
    private static string GenerateCodeVerifier() => GenerateRandomString(64);
    ...
    private async Task<AuthLookupModel?> LookupAuthAsync(
        StraumrWorkspace workspace,
        string name,
        StraumrWorkspaceEntry entry,
        CancellationToken cancellationToken)
    {
        foreach (Guid id in workspace.Auths)
        {
            try
            {
                ...
                if (string.Equals(auth.Name, name, StringComparison.OrdinalIgnoreCase))
                {
                    return new AuthLookupModel(id, auth);
                }
            }
    ...
    private async Task<AuthLookupModel> RequireAuthAsync(
        StraumrWorkspace workspace, string name, string errorMessage, StraumrWorkspaceEntry entry,
        CancellationToken cancellationToken)
    {
        AuthLookupModel? lookup = await LookupAuthAsync(workspace, name, entry, cancellationToken);
        if (lookup.HasValue)
        {
            return lookup.Value;
        }
