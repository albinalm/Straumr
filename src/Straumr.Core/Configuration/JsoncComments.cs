...
public readonly record struct JsoncComment(string Path, JsoncCommentPlacement Placement, string Text);

public static class JsoncComments
{
    ...
    public static string Carry(string? previous, string rewritten)
    {
        ...
        IReadOnlyList<JsoncComment> comments = Capture(previous);
        return comments.Count == 0 ? rewritten : Apply(rewritten, comments);
    }

    public static IReadOnlyList<JsoncComment> Capture(string jsonc)
    {
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                if (reader.TokenType == JsonTokenType.Comment)
                {
                    ...
                    {
                        comments.Add(new JsoncComment(lastCompleted, JsoncCommentPlacement.Trailing, text));
                    }
                    else if (rootDone)
                    {
                        comments.Add(new JsoncComment(Root, JsoncCommentPlacement.After, text));
                    }
                    else
                    {
                        pending.Add(text);
                ...
                Frame? parent = stack.Count == 0 ? null : stack.Peek();
    ...
    public static string Apply(string json, IReadOnlyList<JsoncComment> comments)
    {
        if (comments.Count == 0)
        {
            return json;
        ...
        foreach ((string path, JsoncCommentPlacement placement, List<string> texts) in Group(comments))
        {
            ...
            {
                insertions.Add(new Insertion(bytes.Length, "\n" + string.Join("\n", texts)));
                continue;
            }
    ...
    private static Dictionary<string, Slot> ScanSlots(byte[] bytes)
    {
        Dictionary<string, Slot> slots = [];
        Stack<Frame> stack = new();
        int pendingStart = -1;
        ...
        while (reader.Read())
        {
            ...
            {
                Frame closed = stack.Pop();
                Slot container = slots[closed.Path];
    ...
    private static string PathFor(Frame? parent, string? pendingName) =>
        parent is null
            ? Root
            : parent.IsArray
    ...
    private static void Flush(
        List<JsoncComment> into, List<string> pending, string path, JsoncCommentPlacement placement)
    {
        foreach (string text in pending)
        {
            into.Add(new JsoncComment(path, placement, text));
        }
    ...
    private readonly record struct Insertion(int Offset, string Text);
}
