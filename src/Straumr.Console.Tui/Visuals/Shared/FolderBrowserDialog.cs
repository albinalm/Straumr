...
/// <summary>
/// The reusable cross-platform folder browser: navigation, filtering, and the create, rename and
/// delete a native picker offers. Version 3.9.0 has no file or folder dialog, and a native one would
/// cost portability, Native AOT and the TUI's dependency isolation, so this composes the framework's
/// modal with the app's own resource list, inline filter and command bar.
/// </summary>
/// <remarks>
...
internal sealed class FolderBrowserDialog
{
    ...
    /// </summary>
    private readonly State<int> _folderCount = new(0);
        ...
        string confirmLabel = "Select folder")
    {
        ...
        _filter = new ResourceFilter("filter folders", ApplyFilter, () => _folderList);
        ...
        _folderList.ItemActivated += index => Open(_visibleEntries[index]);
        ...
        _folderList.AddCommand(new Command
        {
            Id = "FolderBrowser.Select.Current",
    ...
    private Visual BuildContent(Visual hints, Visual actions) =>
        new Grid()
            ...
            .Cell(
                StraumrSurfaces.TitledDivider("Folders", () => _folderList.HasFocusWithin),
                2,
                0)
            .Cell(
    ...
    private string CountLabel() =>
        _filter.Text.Trim().Length == 0
            ? _folderCount.Value.ToString()
            : $"{_matchCount.Value}/{_folderCount.Value}";
    ...
    private string? Notice()
    {
        ...
        if (query.Length > 0)
            return _matchCount.Value == 0 ? $"No folders match {query}." : null;

        return _folderCount.Value == 0 ? "This folder has no subfolders." : null;
    }
    ...
    private void LoadDirectory(string path, string? preferredPath = null)
    {
        ...
        _entries = entries;
        _folderCount.Value = entries.Count(entry => !entry.IsParent);
    ...
    private static ResourceRow ToRow(DirectoryEntry entry) =>
        new(entry.IsParent ? $"{ParentGlyph}{entry.Name}" : entry.Name);
    ...
    private DirectoryEntry? EditableEntry =>
        SelectedEntry is { IsParent: false, IsDrive: false } entry ? entry : null;
    ...
    private void NavigateTo(string path)
    {
        ...
        {
            _error.Value = "Directory does not exist.";
            return;
        }
    ...
    private void ConfirmSelection() => Confirm(SelectionTarget);
    ...
    private sealed class LocationInput : TextBox
    {
            ...
            Action<string?> showError)
        {
            ...
            AddCommand(new Command
            {
                Id = "FolderBrowser.Location.Cancel",
                LabelMarkup = "Back",
                Gesture = new KeyGesture(TerminalKey.Escape),
                Importance = CommandImportance.Primary,
                Presentation = CommandPresentation.CommandBar,
        ...
        private void Complete()
        {
            ...
            try
            {
                _completions = FindCompletions(text, _currentPath());
            }
            catch (Exception exception) when (
                exception is IOException or UnauthorizedAccessException or ArgumentException or
                    NotSupportedException or PathTooLongException)
        ...
        {
            string completed = _completions.Length == 1
                ? Path.EndsInDirectorySeparator(path) ? path : path + Path.DirectorySeparatorChar
                : path;
            Text = completed;
            CaretIndex = completed.Length;
            _completedText = completed;
        }

        private static string[] FindCompletions(string text, string currentDirectory)
        {
            string fullPath = Path.GetFullPath(text.Length == 0 ? "." : text, currentDirectory);
    ...
    private sealed record DirectoryEntry(string Name, string Path, bool IsParent, bool IsDrive)
    {
        public static DirectoryEntry Folder(string path) =>
            new(Label(path), path, IsParent: false, IsDrive: false);

        public static DirectoryEntry Parent(string path) =>
            new(Label(path), path, IsParent: true, IsDrive: false);
        ...
        public static DirectoryEntry Drive(string path) =>
            new(Label(path), path, IsParent: false, IsDrive: true);
