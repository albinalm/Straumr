...
internal static class JsonHighlighting
{
    public static StyledRun[] Line(string line)
    {
        ...
        while (index < line.Length)
        {
            ...
            if (character == '"')
            {
                ...
                style = Names(line, index) ? StraumrStyles.CodeKey : StraumrStyles.CodeString;
            }
            else if (character is '-' or '+' || char.IsAsciiDigit(character))
            {
                ...
                while (index < line.Length && IsNumberPart(line[index]))
                    index++;
                style = StraumrStyles.CodeNumber;
            }
            ...
            {
                index++;
                style = StraumrStyles.CodePunctuation;
            }
            else
            {
                index++;
    ...
    private static Style Word(ReadOnlySpan<char> word) =>
        word.SequenceEqual("true") || word.SequenceEqual("false") ? StraumrStyles.CodeBoolean
        : word.SequenceEqual("null") ? StraumrStyles.CodeNull
        : StraumrStyles.CodePlain;
