...
Gestures resolving upwards is also why a text input may not sit under a bare-letter command without care. The filter inside a preview pane's headers tabs has `Request.Send` (`s`), `PreviewPane.NextTab` (`t`), `Request.Fullscreen` (`v`) and the command prompt's `:` above it, and each would swallow a keystroke meant for the box. Those commands guard on `FocusScopeHelpers.IsTyping` with `ConsumesGestureWhenUnavailable = false`, so the gesture falls through to text input while any editor holds focus. A new command over a pane that contains an input needs the same pair.
